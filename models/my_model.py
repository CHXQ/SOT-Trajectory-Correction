...
class MyModel(my_base_model.BaseModel):
    ...
    def forward(self, input_dict):
        ...
        # output = points_utils.get_offset_box_tensor(track_bbox, output_offset)
    ...
    def compute_loss(self, data, output):
        ...
        # loss_total = 0.1 * loss_seg + loss_center + 5 * loss_angle
    ...
    def evaluate_one_sequence(self, sequence):
        ...
        # np.save('train_save_bbox/' + str(self.index) + '_input_bbox.npy', np.array(bbox_corners))
        ...
        # np.save('train_save_bbox/' + str(self.index) + '_pc.npy', np.array(pc))
        # gt_corners = [bbs.corners() for bbs in bbox_label]
        # result_corners = [bbs.corners() for bbs in results_bbs]
        # np.save('train_save_bbox/' + str(self.index) + '_gt_bbox.npy', np.array(gt_corners))
        # np.save('train_save_bbox/' + str(self.index) + '_output_bbox.npy', np.array(result_corners))
        # gt_centers = [bbs.center for bbs in bbox_label]
        # np.save('val_save_bbox/' + str(self.index) + '_gt_center.npy', np.array(gt_centers))
